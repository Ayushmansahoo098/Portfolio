...
import { Terminal as TerminalIcon, CheckCircle2, CornerDownLeft } from 'lucide-react';
...
"Let's build intelligent systems together."`;
  ...
  return (
    <div className="w-full glass-panel rounded-xl border border-white/10 shadow-2xl overflow-hidden font-mono text-xs text-slate-200 bg-[#080305]">
      {/* Terminal Titlebar */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 bg-[#120508] border-b border-white/10">
        <div className="flex items-center space-x-2 truncate">
          <TerminalIcon className="w-4 h-4 text-[#B8002E] shrink-0" />
          <span className="text-white font-semibold text-[11px] sm:text-xs tracking-wider truncate">
            AYUSHMAN // TERMINAL
          </span>
          {isSudoGranted && (
            <span className="hidden xs:flex items-center space-x-1 bg-[#6D001A] text-white border border-[#990026] text-[9px] px-1.5 py-0.5 rounded-full font-bold shrink-0">
        ...
        <div className="flex items-center space-x-2 shrink-0">
          ...
          <div className="flex space-x-1 ml-1 sm:ml-2">
            <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#6D001A] inline-block"></span>
            <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#990026] inline-block"></span>
            <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-white inline-block"></span>
          </div>
        </div>
      </div>

      {/* Quick Action Chips */}
      <div className="p-2 sm:p-2.5 bg-[#050103] border-b border-white/5 flex flex-wrap gap-1 sm:gap-1.5">
        <span className="text-[9px] sm:text-[10px] text-slate-500 self-center mr-1">PRESETS:</span>
        {presetButtons.map((cmd) => (
          <button
            key={cmd}
            onClick={() => handleRunCommand(cmd)}
            data-cursor="RUN CMD"
            className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[10px] sm:text-[11px] transition-all border ${
        ...
        {history.map((item, idx) => (
          <div key={idx} className="space-y-1">
            <div className="flex items-center space-x-1.5 text-[#B8002E]">
              <span className="text-slate-500 text-[10px]">ayushman@os:~$</span>
              <span className="font-bold text-white">{item.cmd}</span>
            </div>
            <pre
              className={`whitespace-pre-wrap leading-relaxed p-2 sm:p-2.5 rounded-lg border text-[10px] sm:text-[11px] ${
                item.isSecret
      ...
      <form
        ...
        className="flex items-center px-3 sm:px-4 py-2 sm:py-2.5 bg-[#120508] border-t border-white/10 text-[11px] sm:text-xs"
      >
        <span className="text-[#B8002E] mr-1.5 shrink-0">ayushman@os:~$</span>
        <input
          type="text"
          value={inputVal}
          onChange={(e) => setInputVal(e.target.value)}
          placeholder="type 'whoami', 'projects'..."
          className="w-full bg-transparent text-white placeholder-slate-600 focus:outline-none text-[11px] sm:text-xs font-mono"
        />
        <button type="submit" className="text-slate-500 hover:text-white ml-2 shrink-0">
