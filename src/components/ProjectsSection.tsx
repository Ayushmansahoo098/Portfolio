    ...
    <section id="projects" className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2 text-[#E01E43] font-mono text-xs font-semibold uppercase tracking-widest">
          ...
          <span>CHAPTER 03 // FEATURED WORK</span>
        </div>
        ...
        {filteredProjects.map((project) => (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="px-3 py-1 rounded-full bg-[#4A0012]/80 text-white border border-[#990026]/40 text-xs font-mono font-semibold">
                {project.category}
              </span>
            ...
            <div className="space-y-3">
              <h3 className="text-2xl font-extrabold text-white group-hover:text-[#E01E43] transition-colors flex items-center justify-between">
                <span>{project.name}</span>
                <ChevronRight className="w-5 h-5 text-slate-500 group-hover:text-[#E01E43] group-hover:translate-x-1 transition-all" />
              </h3>
      ...
      <AnimatePresence>
        {activeModalProject && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-xl overflow-y-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: 30 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 30 }}
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
              className="glass-panel w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 sm:p-10 rounded-2xl border border-[#990026]/60 bg-[#080305] shadow-2xl space-y-8 relative font-sans my-auto"
            >
              {/* Close Button */}
              <button
                onClick={closeModal}
                className="absolute top-6 right-6 text-slate-400 hover:text-white p-2 rounded-xl bg-[#4A0012]/60 hover:bg-[#6D001A] transition-colors border border-[#990026]/40"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-4 pr-12">
                <div className="flex items-center space-x-3 font-mono text-xs">
                  <span className="px-3 py-1 rounded-full bg-[#6D001A] text-white border border-[#990026] font-bold">
                    {activeModalProject.category}
                  </span>
                  <span className="text-slate-500">Year {activeModalProject.year}</span>
                </div>
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                {activeModalProject.liveDemoUrl && (
                  <a
                    ...
                    className="px-5 py-2.5 rounded-xl bg-[#6D001A] hover:bg-[#8E0022] text-white font-mono font-bold text-xs flex items-center space-x-2 transition-all shadow-lg shadow-[#6D001A]/30 border border-[#990026]"
                  >
                    <span>LIVE DEMO</span>
                ...
                {activeModalProject.sourceCodeUrl && (
                  <a
                    ...
                    rel="noreferrer"
                    data-cursor="GITHUB"
                    className="px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-200 border border-white/10 font-mono font-semibold text-xs flex items-center space-x-2 transition-all"
                  >
              ...
              {activeModalProject.metrics && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-xl bg-[#120508] border border-white/10 font-mono">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm leading-relaxed">
                ...
                <div className="p-5 rounded-xl bg-[#120508] border border-white/5 space-y-3">
              ...
              <div className="space-y-4 border-t border-white/10 pt-6">
                ...
                <div className="p-4 rounded-xl bg-[#4A0012]/40 border border-[#990026] text-white text-sm font-sans flex items-start space-x-3">
              ...
              <div className="pt-4 border-t border-white/10 space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeModalProject.technologies.map((t) => (
                    <span
                      key={t}
                      className="px-3 py-1 rounded bg-[#120508] border border-white/10 text-white font-mono text-xs"
                    >
                      {t}
                    </span>
                  ))}
